...
const MasterCalendarView = lazy(() => import("./views/MasterCalendarView").then(m => ({ default: m.MasterCalendarView })));
const ManpowerCalendarView = lazy(() => import("./views/ManpowerCalendarView").then(m => ({ default: m.ManpowerCalendarView })));
...
const PerformanceAnalyticsView = lazy(() => import("./views/PerformanceAnalyticsView").then(m => ({ default: m.PerformanceAnalyticsView })));
...
const MeterQALogsView = lazy(() => import("./views/MeterQALogsView").then(m => ({ default: m.MeterQALogsView })));
import {
  WifiOff,
  Wifi,
  Zap,
...
export default function App() {
  ...
  useEffect(() => {
    if (currentUid && localStorage.getItem("mockDataCleanedUp3") !== "true") {
      removeMockDataFromFirestore(currentUid);
    }
  }, [currentUid]);
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col justify-center items-center p-4">
        <div className="w-[90%] max-w-[448px] bg-surface border border-outline-variant rounded-2xl shadow-lg p-8 flex flex-col items-center">
          ...
          <p className="text-on-surface-variant text-center text-sm mb-6">Biometric verification failed. Please try again or sign out to use a password.</p>
          <div className="flex flex-col gap-3 w-full">
            <button
              onClick={async () => {
                setBiometricStatus('checking');
                if (userEmailForBiometric && await verifyBiometric(userEmailForBiometric)) {
                  setBiometricStatus('verified');
              ...
              className="w-full bg-primary text-on-primary py-3 px-4 rounded-xl font-semibold hover:bg-primary/90 transition-colors"
            >
              Try Again
            </button>
            <button
              ...
              className="w-full bg-transparent border border-outline text-primary py-3 px-4 rounded-xl font-semibold hover:bg-surface-variant transition-colors"
            >
              Use Password
            </button>
          </div>
      ...
      <main className="flex-1 min-w-0 md:ml-64 flex flex-col h-full overflow-y-auto print:overflow-visible print:h-auto w-full pb-[68px] md:pb-0 relative printable-area">
        ...
        <div className="flex-1 w-full mx-auto pb-6">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex flex-col gap-4">
                    {syncConflicts.map(conflict => (
                      <div key={conflict.id} className="bg-surface rounded-lg border border-error/30 p-4 shadow-sm">
          ...
          <Suspense fallback={<div className="p-6 space-y-4 animate-pulse"><div className="h-8 bg-surface-container-high rounded w-1/4"></div><div className="h-32 bg-surface-container-high rounded"></div><div className="grid grid-cols-1 md:grid-cols-3 gap-4"><div className="h-24 bg-surface-container-high rounded"></div><div className="h-24 bg-surface-container-high rounded"></div><div className="h-24 bg-surface-container-high rounded"></div></div></div>}>
            ...
            {activeTab === "meter_qa_logs" && (
              <MeterQALogsView currentUid={currentUid} />
            )}
            ...
            {activeTab === "staff" && (
              <StaffView setActiveTab={setActiveTab} currentUser={currentUser} currentUid={currentUid} currentUserRole={currentUserRole} globalSearchQuery={globalSearchQuery} />
            )}
            {activeTab === "live-map" && (
            ...
            {activeTab === "settings" && <SettingsView isDarkMode={isDarkMode} toggleTheme={toggleTheme} userEmail={userEmailForBiometric} currentUserRole={currentUserRole} />}
          </Suspense>
        </div>
      </main>
