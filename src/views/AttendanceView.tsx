...
const DEFAULT_SHIFTS: Record<string, ShiftDefinition> = {
  ...
  ERASE: {
    ...
    name: "Clear Cell",
    time: "Remove shift",
    color: "bg-surface text-outline border border-dashed border-outline-variant",
    endHour: -1,
    startHourDay: -1,
  },
  "": {
...
export function AttendanceView({
  ...
  const handleDeleteShift = (key: string) => {
    ...
    if (inUse) {
      alert(`Warning: The shift "${palette[key].label}" is currently assigned to some staff. Please unassign before deleting.`);
      return;
    }
            ...
            <tbody className="divide-y divide-outline-variant/40 bg-surface">
              {customStaff.map((staff) => {
                ...
                return (
                  ...
                  >
                    <td className="border-r border-outline-variant py-1 px-3 shadow-[1px_0_0_0_#e5e7eb] sticky left-0 z-10 bg-surface group-hover:bg-surface-container-lowest">
                    ...
                    <td className="py-1 px-3 text-center bg-surface group-hover:bg-surface-container-lowest">
