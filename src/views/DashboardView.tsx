...
import { HuddleReportModal } from "../components/HuddleReportModal";
...
export function DashboardView({
  ...
  const recentActivityLogs = useMemo(() => {
    ...
    const getValidDate = (ts: any) => {
      ...
      if (ts.seconds) return ts.toDate().toISOString();
  ...
  const myShifts = upcomingShifts.filter(s => 
    !currentUser || 
    (s.assignedTo && s.assignedTo.toLowerCase().includes(currentUser.toLowerCase())) ||
    (s.title && s.title.toLowerCase().includes(currentUser.toLowerCase()))
  );
    ...
    <div className="max-w-7xl mx-auto px-margin-mobile lg:px-margin-desktop py-md space-y-lg mb-20 animate-in fade-in zoom-in-95 duration-300">
      <div className="flex justify-between items-center mb-2">
        ...
        <div className="flex items-center gap-3">
          <label className="flex items-center gap-2 cursor-pointer bg-surface-container-low px-3 py-1.5 rounded-full border border-outline-variant hover:bg-surface-container-high transition-colors">
            <input 
              type="checkbox"
              checked={isHuddleMode}
              onChange={(e) => setIsHuddleMode(e.target.checked)}
              className="rounded text-primary focus:ring-primary border-outline"
            />
            <span className="text-sm font-semibold text-on-surface">Huddle Mode</span>
          </label>
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-outline-variant p-6 mt-6 animate-in fade-in slide-in-from-bottom-4">
          ...
          <form
            onSubmit={async (e) => {
              ...
              const statusUpdate = (form.elements.namedItem("statusUpdate") as HTMLTextAreaElement).value;
              const shiftBlockers = (form.elements.namedItem("shiftBlockers") as HTMLTextAreaElement).value;
              const safetyNotes = (form.elements.namedItem("safetyNotes") as HTMLTextAreaElement).value;
              ...
              form.reset();
              alert("Huddle report submitted!");
            }}
            className="flex flex-col gap-4"
          >
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-semibold text-on-surface">Status Update</label>
              <textarea name="statusUpdate" required rows={3} className="w-full rounded-lg border border-outline-variant p-3 text-sm focus:ring-primary focus:border-transparent outline-none" placeholder="What was accomplished today?"></textarea>
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-semibold text-on-surface text-error">Shift Blockers</label>
              <textarea name="shiftBlockers" rows={2} className="w-full rounded-lg border border-outline-variant p-3 text-sm focus:ring-primary focus:border-transparent outline-none" placeholder="Any issues or blockers?"></textarea>
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-semibold text-on-surface text-secondary">Safety & Observations</label>
              <textarea name="safetyNotes" rows={2} className="w-full rounded-lg border border-outline-variant p-3 text-sm focus:ring-primary focus:border-transparent outline-none" placeholder="Safety notes..."></textarea>
            </div>
            <button type="submit" className="bg-primary text-white py-3 rounded-lg font-bold mt-2">Submit Report</button>
          </form>
        </div>
      ) : (
        <>
                  ...
                  <div key={log.id} className="p-4 flex items-start gap-4 hover:bg-surface-container-low transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex justify-between items-center gap-2">
                        ...
                        <span className="text-xs font-medium text-primary shrink-0 bg-primary/10 px-2 py-0.5 rounded-full">
                          {log.user}
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="h-[200px] w-full flex items-center justify-center relative">
                    ...
                    <div className="absolute text-center mt-10">
                      <span className="text-4xl font-bold text-on-surface">
                        {activityCompliance[0].value}%
                      </span>
                      <span className="block text-xs font-semibold text-on-surface-variant">
                        Completed ({activitySpecificTasks.filter(t => t.status === 'completed' || t.status === 'done').length}/{activitySpecificTasks.length || 1})
                      </span>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4 w-full mt-4">
                    <div className="bg-surface-container-low p-2 rounded-lg text-center">
                      <div className="text-xl font-bold text-on-surface">
                        {activitySpecificTasks.length > 0 ? (activitySpecificTasks.length / 30).toFixed(1) : "0.0"}
                      </div>
      ...
      <HuddleReportModal
        ...
        onClose={() => setIsHuddleModalOpen(false)}
