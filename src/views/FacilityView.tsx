...
export function FacilityView({ currentUid, setActiveTab, globalSearchQuery = "" }: FacilityViewProps) {
  ...
  if (selectedFacility) {
    ...
    return (
      <div className="flex flex-col h-full bg-surface">
        <header className="bg-surface border-b border-outline-variant px-4 py-4 flex items-center gap-3 sticky top-0 z-20">
          <button 
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 lg:max-w-6xl mx-auto w-full space-y-6 pb-24">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="md:col-span-2 space-y-6">
              ...
              <div className="bg-surface-container-low border border-outline-variant rounded-2xl overflow-hidden shadow-sm h-48 relative flex items-center justify-center group cursor-crosshair">
                ...
                <div className="text-center z-10 flex flex-col items-center">
                  ...
                  <span className="font-mono text-[10px] text-outline px-2 py-0.5 rounded bg-surface">10°{(Math.random() * 5 + 40).toFixed(4)}' N, 122°{(Math.random() * 2 + 30).toFixed(4)}' E</span>
                </div>
                <div className="absolute top-3 right-3 text-[10px] uppercase font-bold tracking-wider text-outline bg-surface px-2 py-0.5 rounded-md shadow-sm opacity-80 group-hover:opacity-100 transition-opacity">GPS Signal Locked</div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {assets.map((asset, idx) => {
                    ...
                    return (
                      <div key={idx} className="bg-surface border border-outline-variant p-4 rounded-xl shadow-sm flex flex-col">
            ...
            <div className="md:col-span-1">
              <div className="bg-surface border border-outline-variant rounded-2xl shadow-sm overflow-hidden flex flex-col h-full min-h-[500px]">
  ...
  return (
    <div className="flex flex-col h-full bg-surface">
        <header className="bg-surface border-b border-outline-variant px-4 py-4 flex items-center justify-between sticky top-0 z-20">
          <div>
        ...
        <div className="p-4 bg-surface border-b border-outline-variant sticky top-[73px] z-10 flex flex-col sm:flex-row gap-3 items-center">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {filteredFacs.map(f => (
                    <div 
                        ...
                        className="bg-surface border border-outline-variant p-4 rounded-2xl shadow-sm hover:shadow-md hover:border-primary/40 cursor-pointer transition-all active:scale-[0.98] group"
                    >
