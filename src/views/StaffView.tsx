...
export function StaffView({ currentUser, currentUid, currentUserRole, setActiveTab, globalSearchQuery = "" }: StaffViewProps) {
      ...
      (snapshot) => {
        if (snapshot.empty) {
          ...
          const staffDocs = snapshot.docs.map((d) => {
            const rawName = d.data().name as string || "";
            const split = rawName.split(" - ");
            return {
              id: d.id,
              name: split[0],
              position: d.data().position as string || (split.length > 1 ? split[1] : ""),
              email: d.data().email as string || "",
              role: d.data().role as string || "tech",
  ...
  const startEdit = (person: StaffMember) => {
    ...
    setEditValue(splitName[0]);
    setEditPosition(person.position || (splitName.length > 1 ? splitName[1] : ""));
    setEditEmail(person.email || "");
    setEditRole(person.role || "tech");
  ...
  const saveEdit = async (person: StaffMember) => {
    ...
    const finalPosition = editPosition.trim();
    ...
    if (currentUid) {
      ...
      const updated = staffObjList.map((s) =>
        s.id === person.id
          ? { ...s, name: finalName, position: finalPosition, email: finalEmail, role: finalRole, reportsTo: finalReportsTo, status: finalStatus }
          : s,
      );
        ...
        <div className="bg-surface border border-outline-variant p-4 rounded-xl flex items-center gap-4 hover:border-primary/50 hover:shadow-sm transition-all mb-3 relative z-10 w-full md:w-auto overflow-hidden">
          ...
          {isEditing ? (
            ...
            <div className="flex-col min-w-0 flex-1">
              ...
              {person.position && (
                <div className="text-xs text-on-surface-variant truncate" title={person.position}>
                  {person.position} {person.email ? `• ${person.email}` : ""}
                </div>
              )}
              {!person.position && person.email && (
                <div className="text-xs text-on-surface-variant truncate" title={person.email}>
                  {person.email}
                </div>
              )}
            </div>
  ...
  if (currentUserRole && !['admin', 'operations_manager', 'technical_head'].includes(currentUserRole.toLowerCase()) && !currentUserRole.toLowerCase().includes('head')) {
    return (
      <div className="p-6 max-w-7xl mx-auto flex flex-col items-center justify-center min-h-[50vh] text-center">
        ...
        <p className="text-on-surface-variant max-w-[448px]">
          The team management module is restricted to administrators and operations managers.
        </p>
      </div>
    );
  }
