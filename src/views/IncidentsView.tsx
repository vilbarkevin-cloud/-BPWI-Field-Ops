...
interface IncidentsViewProps {
  ...
  onSwitchToAdhoc?: (activityType?: string, context?: any) => void;
  initialContext?: any;
  isOnline?: boolean;
}

export function IncidentsView({ currentUid, currentUser, setActiveTab, onSwitchToAdhoc, initialContext, isOnline = true }: IncidentsViewProps) {
  ...
  const [facility, setFacility] = useState(initialContext?.facility || "");
  const [joNumber, setJoNumber] = useState(initialContext?.joNumber || "");
  const [accountNumber, setAccountNumber] = useState(initialContext?.accountNumber || "");
  const [site, setSite] = useState(initialContext?.site || "");
  const [blockLot, setBlockLot] = useState(initialContext?.blockLot || "");
  ...
  const lastParsedAccount = React.useRef("");
  ...
  const handleIssueJobOrder = async (inc: any) => {
    if (!currentUid) {
    ...
    try {
      await setDoc(doc(db, `users/${currentUid}/tasks`, taskId), {
        title: `Resolve Incident: ${inc.type?.replace('_', ' ') || 'Issue'}`,
        description: inc.summary || `Autogenerated task from incident ${inc.id}`,
        priority: inc.severity === 'critical' ? 'high' : inc.severity === 'high' ? 'high' : 'medium',
        status: 'pending',
        facility: inc.facility || '',
        site: inc.site || '',
        blockLot: inc.blockLot || '',
        joNumber: inc.id,
      ...
      showToast("Task created successfully! Check Tasks tab.", "success");
      if (setActiveTab) {
        setActiveTab("tasks");
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (isOnlineRef.current && !isOnline && facility && summary && !isSubmitting && !submitted) {
      showToast("Connection lost. Auto-saving draft locally...", "warning");
      handleSubmit();
    }
    isOnlineRef.current = isOnline;
  }, [isOnline, facility, summary, isSubmitting, submitted]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!isOnline) {
        await enqueueAction("watsanIncidents", {
          id: docRef.id,
          ...incidentData,
          isSynced: false,
          createdAt: new Date().toISOString(),
      ...
      showToast(
        !isOnline
          ? "Offline: Incident queued for synchronization."
                ...
                <div className="p-lg grid grid-cols-1 md:grid-cols-2 gap-lg">
                  ...
                  <div className="space-y-sm">
                    ...
                    <datalist id="facilities-list">
                      {[...new Set([...defaultFacilities, ...customAreas])].map((f) => (
                  ...
                  <div className="space-y-sm">
                    ...
                    <datalist id="site-list">
                      {/* TODO: Site list based on facility */}
                    </datalist>
                  </div>
          ...
          <div className="bg-surface-container-low border border-outline-variant rounded-xl p-lg h-full">
            ...
            <div className="space-y-md">
              {recentIncidents.slice(0, 5).map((inc) => (
                <div key={inc.id} className="p-md bg-surface border border-outline-variant rounded-lg">
                  <div className="flex items-center justify-between mb-xs">
                    <span className="bg-primary-container text-on-primary-container px-2 py-0.5 rounded text-label-sm font-label-sm truncate max-w-[120px]">
                      User
                    </span>
                    <span className="font-label-sm text-label-sm text-outline shrink-0">
                      {inc.createdAt?.seconds 
                        ? new Date(inc.createdAt.seconds * 1000).toLocaleString(undefined, { 
                  ...
                  <p className="font-body-md text-body-md text-on-surface">
                    <strong>{inc.reportedBy || "System"}</strong> logged {inc.type ? inc.type.replace('_', ' ') : 'incident'} at {inc.facility} - <span className={
                      inc.severity === 'critical' ? 'text-error font-bold' : 
                      inc.severity === 'high' ? 'text-orange-500 font-bold' : 'text-on-surface'
                    }>{inc.severity} severity</span>.
                  </p>
                  {setActiveTab && (
                    <button 
                      onClick={() => handleIssueJobOrder(inc)}
                      className="mt-2 text-sm text-primary hover:text-primary-container font-medium"
                    >
                      Issue Job Order / Create Task &rarr;
                    </button>
                  )}
                </div>
              ))}
            ...
            <div className="mt-lg rounded-xl overflow-hidden border border-outline-variant relative h-48 group cursor-crosshair">
              ...
              <div className="absolute bottom-3 left-3 bg-surface/90 text-on-surface backdrop-blur-md px-3 py-1 rounded-full text-label-sm font-label-sm shadow-sm flex items-center gap-1">
