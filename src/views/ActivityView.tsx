...
export function ActivityView({
  ...
  useEffect(() => {
    ...
    if (pendingPMS) {
      try {
        const data = JSON.parse(pendingPMS);
        setWorkCategory("ad-hoc");
        if (data.area) setSelectedArea(data.area);
        if (data.activityType) setSelectedActivity(data.activityType);
        if (data.pmsId) setPmsIdToComplete(data.pmsId);
        localStorage.removeItem("pendingPMSActivity");
      } catch (e) {
        console.error("Error parsing pending PMS:", e);
      }
    }
  }, []);
  ...
  const handlePhotoSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (newPhotosBase64.length > 0) {
      setPhotosBase64(prev => [...prev, ...newPhotosBase64]);
    }
  ...
  const { fuelChartData, lastPmsDate } = useMemo(() => {
    const gensetHistory = siteHistory
      .filter((h) => h.type === "genset_monitoring")
  ...
  const finalizeSubmit = async (
    ...
    try {
      ...
      if (selectedActivity === "leak_detection" || leakIndication === "Y") {
        ...
        setDoc(doc(db, `users/${currentUid}/tasks`, taskId), {
          ...
          location: selectedSiteOrWell || selectedArea || "Field",
          deadline: "Pending Assignment",
          description: `Leak detected at ${blockLot || "unknown location"} during ${selectedActivity === "leak_detection" ? "Leak Detection" : "other activity"}. Requires immediate follow-up.`,
          assignedTo: "Unassigned",
          status: "pending",
          isSynced: navigator.onLine,
          createdAt: serverTimestamp(),
      ...
      const activityData: any = {
        ...
        details: {
          ...
          ...(isMeterReplacement || isMeterInstallation
            ...
            : {}),
          ...(selectedActivity === "genset_monitoring"
            ? {
                gensetCause,
                gensetRefillLevel,
                didRefillFuel,
  ...
  const isPumpOrGenset = ["pump_monitoring", "genset_monitoring"].includes(
    selectedActivity || "",
  );
  const isTankActivity = ["tank_cleaning", "tank_opening"].includes(
    selectedActivity || "",
  );
  ...
  return (
    <div className="max-w-4xl mx-auto px-margin-mobile mt-lg space-y-gutter pb-24 animate-in fade-in duration-300">
      <div className="flex flex-col gap-0.5">
        <h2 className="text-headline-md font-semibold text-on-surface flex items-center justify-between">
          <span>Work Logs</span>
          <button 
            type="button"
            onClick={async () => {
              if (navigator.geolocation && currentUid) {
                navigator.geolocation.getCurrentPosition(async (pos) => {
                  try {
                    await setDoc(doc(db, `users/${currentUid}/activities`, `clock-${Date.now()}`), {
                      userId: currentUid,
                      type: "attendance",
                      action: "clock-in",
                      date: new Date().toISOString(),
            ...
            className="text-sm bg-primary/10 text-primary px-3 py-1.5 rounded-full font-semibold hover:bg-primary/20 transition-colors flex items-center gap-1"
          >
            <Clock className="w-4 h-4" /> Clock In
          </button>
        </h2>
        <p className="text-on-surface-variant text-label-md">
          Log a new accomplishment, task, or incident.
        </p>
      </div>
      ...
      <div className="flex gap-2 bg-surface-container-low p-2 rounded-xl border border-outline-variant">
        {[
          { id: "ad-hoc", label: "Ad-hoc Activity" },
          { id: "task", label: "Task" },
          ...
          <button
            ...
            type="button"
            onClick={() => {
              setWorkCategory(cat.id as any);
              if (cat.id === "task") setSelectedActivity("task");
              if (cat.id === "incident") setSelectedActivity("incident");
              if (cat.id === "ad-hoc" && (selectedActivity === "task" || selectedActivity === "incident")) {
                setSelectedActivity(activityTypes[0]?.id || "");
              }
            }}
      ...
      >
        {/* Selection List for Activity Types - ONLY SHOW FOR AD-HOC */}
        {workCategory === "ad-hoc" && (
          <div className="bg-surface border border-outline-variant rounded-2xl shadow-sm p-4 relative z-20 overflow-hidden flex flex-col">
        ...
        >
          ...
          <div className="bg-surface border border-outline-variant rounded-lg shadow-sm p-4 relative z-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 relative">
              ...
              <div className="flex flex-col gap-xs relative md:col-span-1">
                ...
                <div
                  className="min-h-[42px] cursor-pointer rounded bg-surface border border-outline-variant focus-within:ring-2 focus-within:ring-primary focus-within:border-transparent text-body-md py-2 px-3 flex flex-wrap gap-2 items-center"
                ...
                {staffDropdownOpen && (
                  <div className="absolute top-full left-0 right-0 mt-1 max-h-80 overflow-y-auto bg-surface-container-lowest border border-outline-variant rounded-lg shadow-xl z-50 p-3 flex flex-col gap-2">
                            ...
                            reading3 !== "" && (
                              <div className="md:col-span-2 mt-4 p-4 bg-surface-container border border-outline-variant rounded-lg relative overflow-hidden">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pb-4 border-b border-outline-variant/30 mb-4">
                                  ...
                                  <div className="bg-surface p-3 rounded border border-outline-variant flex flex-col justify-center items-center">
                                    {(() => {
                                      const totalErr =
          ...
          {/* SECTION: Water Sampling Witness */}
          {selectedActivity === "water_sampling" && (
            <div className="bg-surface border border-outline-variant rounded-lg shadow-sm p-4">
              <h3 className="font-headline-md text-headline-md text-on-surface mb-3 border-b border-outline-variant/30 pb-2">
                Witness Verification (Client Side)
              </h3>
